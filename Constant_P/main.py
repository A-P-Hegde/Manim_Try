...
class constant_phi_surface(ThreeDScene):
    def construct(self):
        # Create 3D Axes
        axes = ThreeDAxes()
        ...
        half_sphere = Surface(
            ...
            resolution=(8, 8),      # Adjust smoothness
            color=BLUE
        ).set_opacity(0.5).set_fill(RED)  # Set transparency if needed
        ...
        smal_surf=Surface(
            lambda u, v: np.array([
                u*np.cos(PI/4)*np.sin(v),  # X-coordinate
                u*np.sin(PI/4)*np.sin(v),  # Y-coordinate
                u*np.cos(v)               # Z-coordinate
                ]),
                u_range=[2.7,3],         #rotation around Z-axis
                v_range=[PI/4,PI/4 + to_rad(10)],        # part of sphere
                resolution=(5, 5),      # Adjust smoothness
        ).set_fill(PINK)

        #here the line sweeps the small area (constant theta area)
        line3=Line3D(ORIGIN,from_polar(3,PI/4,PI/4 + to_rad(10)),resolution=20)
